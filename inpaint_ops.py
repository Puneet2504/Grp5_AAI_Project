...
def standard_conv(x,mask,cnum,ksize=3,stride=1,rate=1,name='conv',padding='SAME'):
        ...
        padding = 'VALID'
        x       = tf.keras.layers.Conv2D(cnum,ksize,stride,dilation_rate=rate,activation=tf.nn.elu
                                  ,padding=padding,name=name+'_1')(x)
        return x
def standard_dconv(x,mask,cnum,name='deconv',padding='VALID'):
        ...
        stride = 1
        shape  = x.get_shape().as_list()
        x      = tf.image.resize(x,[shape[1]*2,shape[2]*2], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        p3     = int(1 * (3 - 1) / 2)
...
# to control the Lipschitz constant of network weights
