...
def region_deconv(x_complete, x_missing, mask,name = 'com_'):
    ...
    mask = tf.image.resize(mask, size = [shape[1], shape[2]], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def region_conv(x_complete, x_missing, mask,name = 'com_'):
    ...
    x_fusion   = tf.keras.layers.Conv2D(shape[-1], kernel_size = 3, strides = 1, padding = 'VALID', dilation_rate = 1, name = name + "_fusion")(x_fusion)
