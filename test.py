...
if __name__=='__main__':
        ...
        images=tf.compat.v1.placeholder(tf.float32,[1,args.height,args.width,3],name = 'image')
        mask=tf.compat.v1.placeholder(tf.float32,[1,args.height,args.width,1],name='mask')
        sess = tf.compat.v1.InteractiveSession()
        inpainting_result=infer(images,mask)
        saver_pre=tf.compat.v1.train.Saver()
        init_op = tf.compat.v1.group(tf.compat.v1.initialize_all_variables(),tf.compat.v1.initialize_local_variables()) 
        sess.run(init_op)
        ...
        cv2.imwrite(args.file_out+"/output.png", img_out[0][...,::-1])
        cv2.imwrite(args.file_out+"/output_masked.png", test_image[0][...,::-1] * test_mask[0] + 255 * (1 - test_mask[0]))
